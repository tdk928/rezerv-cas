...
|---|--------|------|-----------|
| 3 | feature/multi-company-membership | 2026-07-16 | Multi-company: `user_companies` membership; `users.company_id` = активна фирма (JWT). assign-company добавя membership (позволява 2+ фирми); `POST /auth/switch-company` + `UserResponse.companyIds`. |
